...
InitInstructions()
{
  const RectangleType     rect  = {{0,0},{142,294}};
  ...
  // draw the help
...

    {
      ...
      x = 4;
      StrCopy(str,
"Tauchen Sie in der alten Galeere nach antiken Schätzen. Aber lassen \
Sie sich dabei bioß nicht von der hungrigen Riesenkrake erwischen.");
      ptrStr = str;
      ...
      x = 4;
      StrCopy(str,
"Benutzen Sie den Stylus oder die Tasten um den Taucher links und rechts \
zu bewegen und zum Schatz im Boot zu bewegen. Vergessen Sie nicht das \
Gold in das Boot zu bringen. Sie nicht das Gold in das Boot zu bringen.");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
      ...
      x = 4;
      StrCopy(str,
"Spiel A ist für Anfänger, Spiel B für Fortgeschrittene Spieler geeignet.");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
